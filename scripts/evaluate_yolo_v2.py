"""计算测试集 mAP@0.5 和每类 AP（v2 增强模型评估）。"""
...
WEIGHTS = ROOT / "runs" / "train" / "pcb_v2" / "weights" / "best.pt"
TEST_IMAGES = ROOT / "outputs" / "pcb_yolo_dataset_v2" / "images" / "test"
TEST_LABELS = ROOT / "outputs" / "pcb_yolo_dataset_v2" / "labels" / "test"
OUT_DIR = ROOT / "outputs" / "eval_v2"
...
def main():
    if not WEIGHTS.exists():
        raise FileNotFoundError(
            f"没有找到训练权重: {WEIGHTS}\n"
            "请先运行 python scripts/train_yolo_colab.py")
